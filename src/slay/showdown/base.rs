...
impl CurrentShowdown {
    ...
    pub fn take_complete(&mut self) -> Option<Box<dyn ShowDown>> {
        ...
        match self.show_down_type {
            ...
            ShowDownType::OfferChallenges => {
                self.offer.take().map(|x| Box::new(x) as Box<dyn ShowDown>)
            }
        ...
        ModificationTask {
            choices_to_assign: (0..game.number_of_players())
                ...
                .map(|player_index| {
                    (
                        player_index,
                        current.create_choice_for(context, &game.players[player_index]),
                    )
                })
                .collect(),
        }
    ...
    pub(crate) fn set_player_completion(
        &mut self,
        player_index: usize,
        persist: super::completion::RollCompletion,
    ) -> SlayResult<()> {
        match self.show_down_type {
...
pub trait ShowDown: Debug + dyn_clone::DynClone {
    ...
    fn assign_all_choices(
        ...
        for player in game.players.iter_mut() {
            player.choices = Some(self.create_choice_for(context, player));
        }
    }
